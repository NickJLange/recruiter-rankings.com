...
Rails.configuration.x.public_min_reviews = (ENV["PUBLIC_MIN_REVIEWS"].presence || (Rails.env.development? ? "1" : "5")).to_i
Rails.configuration.x.demo_auto_approve = ActiveModel::Type::Boolean.new.cast(ENV.fetch("DEMO_AUTO_APPROVE", Rails.env.development?.to_s))
